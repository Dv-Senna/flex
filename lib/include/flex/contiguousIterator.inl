...
namespace flex {
	...
	template <typename T, typename Cont>
	constexpr ReverseContiguousIterator<T, Cont>::ReverseContiguousIterator(pointer ptr, const Container *container) noexcept :
		m_ptr {ptr},
		m_container {container}
	{
	...
	template <typename T, typename Cont>
	constexpr auto ReverseContiguousIterator<T, Cont>::operator+=(difference_type value) noexcept -> ReverseContiguousIterator<T, Cont>& {
		m_ptr -= value;
		return *this;
	}
	...
	template <typename T, typename Cont>
	constexpr auto ReverseContiguousIterator<T, Cont>::operator+(difference_type value) const noexcept -> ReverseContiguousIterator<T, Cont> {
		auto tmp {*this};
		return tmp += value;
	}
	...
	template <typename T, typename Cont>
	constexpr auto ReverseContiguousIterator<T, Cont>::operator-(const ReverseContiguousIterator<T, Cont> &it) const noexcept -> difference_type {
		if constexpr (flex::config::BUILD_TYPE == flex::BuildType::eDebug) {
			if (m_container != it.m_container)
				return std::numeric_limits<difference_type>::max();
		}
	...
	template <typename T, typename Cont>
	constexpr auto ReverseContiguousIterator<T, Cont>::operator[](difference_type value) const noexcept -> reference {
		return *(m_ptr + value);
	}

	template <typename T, typename Cont>
	constexpr auto ReverseContiguousIterator<T, Cont>::operator*() const noexcept -> reference {
		return *m_ptr;
	}

	template <typename T, typename Cont>
	constexpr auto ReverseContiguousIterator<T, Cont>::operator->() const noexcept -> pointer {
		return m_ptr;
	}
